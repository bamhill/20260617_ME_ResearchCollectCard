...
> 一句话生成杂志风 AI 科研日报。arXiv 顶会 + 32 本期刊 + 19 本 RSS 直连 + 7 个公众号，多路聚合，自动热点提取。
...
**这个工具干三件事：**
1. 多路数据源自动聚合（arXiv 顶会/CS分类 + CrossRef 32本期刊 + RSS直连19本期刊 + 7个公众号）
2. 从全量摘要中自动提取「主题热点」和「方法热点」
3. 渲染成杂志风 HTML 日报，双击即开，一键存图

![效果对比](docs/compare.png)

*左：顶会精选（ICML/NeurIPS/ICLR/ACL/CVPR/AAAI），右：可解释性期刊（KBS/TKDE/EAAI 等 8 刊）*
...
| `journal` | ABS3/4、FMS A/B、CAS 1/2 期刊 | 51 源 (32 CrossRef + 19 RSS) | CrossRef + 期刊RSS |
| `all` | 全部论文源 | 59 | arXiv + CrossRef + 期刊RSS |
| *(wechat)* | 老刘NLP / 专知 / KGraph / 知识图谱科技 / 计算机学报 / AI前沿速递 / 开放知识图谱 | 7 | RSS (WeWe Docker) |
...
├── cli.py                    # typer CLI 入口
├── sources.json              # 66 源配置（数据驱动，改源不改代码）
├── src/
│   ├── fetchers/             # 策略模式（arxiv/crossref/rss，含期刊RSS日期回落+TOC过滤）
